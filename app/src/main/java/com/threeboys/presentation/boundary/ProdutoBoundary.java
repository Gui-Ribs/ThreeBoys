...
public class ProdutoBoundary {
	...
	private ObservableList<String> chocolates = FXCollections.observableArrayList("Ao Leite", "blend", "Branco", "50%");
	private ObservableList<String> tipos = FXCollections.observableArrayList("Normal", "Trufado", "Crocante", "Colher");
	...
	public Pane render() {
		...
		colTamanho.setCellValueFactory(itemData -> new ReadOnlyStringWrapper(itemData.getValue().getTamanho()));
		TableColumn<Produto, String> colChocolate = new TableColumn<>("Chocolate");
		colChocolate.setCellValueFactory(itemData -> new ReadOnlyStringWrapper(itemData.getValue().getChocolate()));
		TableColumn<Produto, String> colTipo = new TableColumn<>("Tipo");
		colTipo.setCellValueFactory(itemData -> new ReadOnlyStringWrapper(itemData.getValue().getTipo()));
		...
		colObservacao.setCellValueFactory(itemData -> new ReadOnlyStringWrapper(itemData.getValue().getTipo()));
